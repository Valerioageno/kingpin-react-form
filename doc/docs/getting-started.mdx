...
Each internal component (`<Input />`, `<Textarea />`) could be thought as just a simple input element so **all the element attributes
...
and the overall form validity (check [validation](./validation) for more).
...
Check the [validation](./validation) documentation for more about the error handling.
